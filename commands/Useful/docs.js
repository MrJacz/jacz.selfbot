...
module.exports = class DocsCommand extends Command {
  ...
  constructor(...args) {
    super(...args, {
      ...
      runIn: ['text', 'dm', 'group'],
      cooldown: 2,
      aliases: ["doc", "d.jsdocs"],
      permLevel: 10,
      botPerms: [],
      requiredSettings: [],
      description: 'Searchs and Parses discord.js docs',
      usage: '<query:str> [version:string]',
      usageDelim: " ",
      extendedHelp: 'No extended help available.'
    });
    // Cache discord.js docs
    this.docs = {};
  ...
  async run(msg, [query, version]) {
    console.log(query) 
    const v = version ? version.toLowerCase() : "stable";
    console.log(v);
    if (!['master', 'stable', 'commando'].includes(v)) return msg.reply(`which version of docs would you like (stable, master, commando)?`);
    // version = version.toLowerCase();
     const docs = await this.fetchDocs(v);
    const [main, member] = this.search(docs, query);

    if (!main) {
      return msg.reply('Could not find that item in the docs.');
    }
    ...
    }[member.category].call(this, main, member, v) : this.formatMain(main, v);

    const icon = 'https://cdn.discordapp.com/icons/222078108977594368/bc226f09db83b9176c64d923ff37010b.webp';
    embed.url = this.getLink(v);
    embed.author = {
      name: version === 'commando' ? 'Commando Docs' : `Discord.js Docs (${v})`,
